...
public class ShoppingView {
    ...
    public ShoppingView(LoginUser user) {
    ...
    public ShoppingView() {
        this.shoppingService = new ShoppingService();
        this.scanner = new Scanner(System.in);
        this.currentUser = new LoginUser("guest", "123", "Indonesia");
    }
    ...
    String userCurrency = shoppingService.getCurrencyByCountry(currentUser.getCountry());
    
    System.out.println("\n=== RINGKASAN CHECKOUT ===");
    System.out.printf("Subtotal produk: %s\n", 
        shoppingService.formatCurrency(shoppingService.convertPrice(subtotal, userCurrency), userCurrency));
    System.out.printf("Ongkos kirim (%s): %s\n", courier.getName(), 
        shoppingService.formatCurrency(shoppingService.convertPrice(shippingCost, userCurrency), userCurrency));
    System.out.printf("Biaya pembayaran (%s): %s\n", payment.getName(), 
        shoppingService.formatCurrency(shoppingService.convertPrice(paymentFee, userCurrency), userCurrency));
    System.out.println("--------------------------------");
    System.out.printf("TOTAL PEMBAYARAN: %s\n", 
        shoppingService.formatCurrency(shoppingService.convertPrice(total, userCurrency), userCurrency));
    ...
    private void printReceipt(double subtotal, Courier courier, double shippingCost,
        ...
        for (int i = 0; i < cartItems.size(); i++) {
            ...
            double convertedPrice = shoppingService.convertPrice(item.getProduct().getPrice(), userCurrency);
            double convertedTotal = shoppingService.convertPrice(item.getTotalPrice(), userCurrency);

            System.out.printf("%-62s \n%2dx @%s",
                    shoppingService.truncateString(item.getProduct().getName(), 62),
                    item.getQuantity(),
                    shoppingService.formatCurrency(convertedPrice, userCurrency));
            System.out.printf("%-15s%s\n", "",
                    shoppingService.formatCurrency(convertedTotal, userCurrency));
        }

        double convertedSubtotal = shoppingService.convertPrice(subtotal, userCurrency);
        double convertedShipping = shoppingService.convertPrice(shippingCost, userCurrency);
        double convertedPaymentFee = shoppingService.convertPrice(paymentFee, userCurrency);
        double convertedTotal = shoppingService.convertPrice(total, userCurrency);

        System.out.println("-".repeat(50));
        System.out.printf("%-35s %s\n", "Subtotal:",
                shoppingService.formatCurrency(convertedSubtotal, userCurrency));
        System.out.printf("%-35s %s\n", "Ongkir (" + courier.getName() + "):",
                shoppingService.formatCurrency(convertedShipping, userCurrency));
        System.out.printf("%-35s %s\n", "Biaya (" + payment.getName() + "):",
                shoppingService.formatCurrency(convertedPaymentFee, userCurrency));
        System.out.println("=".repeat(50));
